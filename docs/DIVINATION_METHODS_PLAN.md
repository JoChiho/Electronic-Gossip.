...
- [ ] **B1** `divinate_manual(upper_idx, lower_idx, changing_line|None)`
- [ ] **B2** 动爻可选「无」→ 全静爻（7/8）
- [ ] **B3** GUI：上卦/下卦 Combobox（乾1…坤8）+ 动爻 Combobox
- [ ] **B4** CLI 非交互参数；`last_method` 持久化
- [ ] **B5** 测试：乾上坤下第3爻动、纯阳静卦等
- [ ] **B6** 版本与文档更新
...
- [ ] **C1** 大衍算法纯函数（分二、挂一、揲四、归奇；三变得一爻）
- [ ] **C2** `divinate_yarrow(rng, *, record_steps=False)` → 六爻 + 可选步骤日志
- [ ] **C3** GUI：「模拟蓍草」+ 可选「演卦过程」展开区
- [ ] **C4** 提示词注明「大衍筮法模拟，非实体蓍草」
- [ ] **C5** 统计测试：固定 seed 可复现；长期分布接近三钱法概率
- [ ] **C6** 版本与文档更新
...
- [ ] **D1** 选定笔画口径（康熙 / 简体）并文档化
- [ ] **D2** `character_to_numbers(text)` → 上卦/下卦/动爻
- [ ] **D3** GUI：一字/词输入 + 笔画预览
- [ ] **D4** 多字策略可配置（首字 / 总笔画等）
- [ ] **D5** 常用字单测与手工核算对照
- [ ] **D6** 版本与文档更新
...
- [ ] **E1** 重构 `divination.py` 为子模块 + 注册表
- [ ] **E2** CLI 交互引导按 `last_method` 分支（对齐 GUI）
- [ ] **E3** README / `PROJECT_STATUS` 起卦方式对照表
- [ ] **E4** 每种新方法 ≥3 单测 + 1 个 service 集成测
- [ ] **E5** GUI 内「起卦方式说明」帮助（Tab 或链接文档）
...
| 文件 | 说明 |
|------|------|
| `bagua/divination.py` | 当前起卦逻辑（待拆分为子模块） |
